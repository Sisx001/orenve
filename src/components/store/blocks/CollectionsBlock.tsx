        ...
        <div className="mt-14 grid gap-px bg-line sm:grid-cols-2 xl:grid-cols-4">
          {collections.map((c, i) => (
            <Reveal key={c.slug} delay={Math.min(i * 0.08, 0.4)} className="bg-paper">
              <LocaleLink href={`/collections/${c.slug}`} className="group relative block h-full overflow-hidden">
                ...
                <span aria-hidden className="absolute inset-0 bg-gradient-to-t from-ink/80 via-ink/10 to-transparent opacity-80 transition-opacity duration-700 group-hover:opacity-95" />
                <span className="absolute inset-x-0 bottom-0 block p-6 text-bone">
                  <span className="display block text-xl">{c.name}</span>
                  <span className="eyebrow mt-1.5 block text-bone/60">{piecesLabel(c.count)}</span>
                  {c.description && (
                    <span className="mt-3 block max-h-0 overflow-hidden text-sm leading-relaxed text-bone/80 opacity-0 transition-all duration-700 ease-editorial group-hover:max-h-28 group-hover:opacity-100">
                      {c.description}
                    </span>
                  )}
                </span>
